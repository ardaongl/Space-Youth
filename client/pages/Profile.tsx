      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-6xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6 lg:sticky lg:top-24 h-[calc(100vh-6rem)] overflow-auto">
            ...
            <div className="bg-card border rounded-lg p-4">
                <div className="h-64 lg:h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={scores.map((s) => ({ subject: s.label, A: s.value }))} outerRadius={95}>
                      <PolarGrid />
            ...
            {/* Career Path with Progress (sticky on lg+) */}
            <div className="border rounded-lg p-4 bg-background shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold">Career Path</h3>
                <div className="text-sm text-muted-foreground">Tamamlanma: %35</div>
              </div>
              ...
              <div className="mt-4 grid gap-4">
                {["Sonuç Sayfası (Hero)", "Foundations", "Prototyping", "Storytelling"].map((step, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="h-8 w-8 rounded-full bg-muted grid place-items-center text-sm font-medium">{i + 1}</div>
                    <div className="flex-1 border rounded-lg p-4">
                      <div className="font-medium">{step}</div>
                      <div className="text-sm text-muted-foreground">Yol adımı açıklaması – ilerleme için görevleri tamamla.</div>
                    </div>
                  </div>
                ))}
              </div>
